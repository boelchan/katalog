        ...
        <div class="card-body">
          ...
          <form action="{{ route('produk.update',$produk->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="kategori_id">
                @foreach($kategoris as $k => $v)
                  <option value="{{ $k }}" @if($k == $produk->kategori_id) {{ 'selected' }} @endif>{{ $v }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="form-group">
              ...
              <input type="text" name="nama" id="" class="form-control" placeholder="Name" required value="{{ $produk->nama }}" />
            </div>

            <div class="form-group">
              <label class="d-block" for="validationBioBootstrap">Deskripsi</label>
              <textarea class="form-control" id="" name="deskripsi" rows="3"  >{{ $produk->deskripsi }}</textarea>
            </div>

            <div class="form-group">
              <label class="form-label" for="">Harga</label>
              <input type="number" name="harga" id="" class="form-control" placeholder="Harga" required value="{{ $produk->harga }}" />
            </div>

            <div class="form-group">
              <label class="d-block">Tampil</label>
              <div class="custom-control custom-radio my-50">
                <input type="radio" id="validationRadio3" name="tampil" class="custom-control-input" required value="1"
                  @if($produk->tampil==1)
                    checked
                  @endif
                />
